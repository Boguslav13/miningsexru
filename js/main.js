...
/*>>>*/
// Forms sending
const formFeedbackElement = document.getElementById('feedbackForm');

// Sending
async function feedbackSend(e) {
    e.preventDefault();

    let validate = formValidate(formFeedbackElement);
    if (validate === 0) {
        let formSuccessField = formFeedbackElement.querySelector('.send-status.success');
        let formFailField = formFeedbackElement.querySelector('.send-status.fail');
        let formNameField = 'feedbackForm';
        let nameField = formFeedbackElement.querySelector('#name').value;
        let emailField = formFeedbackElement.querySelector('#email').value;
        let messageField = formFeedbackElement.querySelector('#message').value;
        ...
        fetch("/form.php", {
            ...
            if (response.ok === true) {
                formSuccessField.style.display = 'block';
            } else {
                formFailField.style.display = 'block';
            }
        }).catch(error =>
            console.error(error)
        );
